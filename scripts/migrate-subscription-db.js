// scripts/migrate-subscription-db.js
const path = require('path');
require('dotenv').config({ path: path.resolve(process.cwd(), '.env.local') });

// Since we can't require TypeScript files directly, we'll inline the schema creation
async function initializeSubscriptionSchema() {
...
async function runMigration() {
  try {
    ...
    console.log('✅ Subscription database migration completed successfully!');
    process.exit(0);
  } catch (error) {
