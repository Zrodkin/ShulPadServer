...
// Replaces Next.js's useSearchParams to read URL query parameters
...
// Replaces Next.js's useRouter for navigation
const useRouter = () => {
  return {
    push: (path: string) => {
      // Navigates to a new page using standard browser API
      window.location.href = path;
    },
  };
};

// Replaces Next.js's Head component to manage the document head
  ...
  useEffect(() => {
    ...
    React.Children.forEach(children, child => {
      if (React.isValidElement(child)) {
        ...
        const elementProps = props as any; // Assert props to 'any' to access properties safely
        ...
        if (elementProps.children && typeof elementProps.children === 'string') {
            element.textContent = elementProps.children;
        }

        // Special handling for the tailwind script to prevent duplicates
        if (tag === 'script' && elementProps.src === 'https://cdn.tailwindcss.com') {
...
// Replaces Next.js's Script component for loading external scripts
    ...
    fetch(`/api/subscriptions/merchant-email?merchant_id=${merchantId}`)
      ...
      .then(data => {
        if (data.merchant_email) {
          ...
          setCustomerEmail(data.merchant_email);
  ...
  if (configLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50">
        <div className="text-center"><Spinner size="12" /><p className="mt-4 text-slate-600">Loading payment configuration...</p></div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen p-4 bg-slate-50">
        <div className="w-full max-w-md p-8 text-center bg-white border border-red-200 rounded-lg shadow-md">
            ...
            <button onClick={() => window.location.reload()} className="px-4 py-2 mt-6 text-sm font-semibold text-white bg-red-600 rounded-md hover:bg-red-700">Retry</button>
        </div>
      </div>
    );
  }
          ...
          <div className="container max-w-6xl px-4 py-12 mx-auto lg:py-20">
            <header className="mb-12 text-center">
              <a href="#" className="inline-flex items-center justify-center w-16 h-16 mx-auto mb-6 bg-indigo-100 rounded-2xl">
                  <svg className="w-8 h-8 text-indigo-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M9 9.563C9 9.252 9.252 9 9.563 9h4.874c.311 0 .563.252.563.563v4.874c0 .311-.252.563-.563.563H9.563C9.252 15 9 14.748 9 14.437V9.563z" /></svg>
              </a>
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="grid gap-8 sm:grid-cols-2">
                      ...
                      <div className="p-8 bg-white border rounded-xl border-slate-200">
                        ...
                        <input type="email" value={customerEmail} onChange={(e) => { setCustomerEmail(e.target.value); setEmailSource(e.target.value === merchantEmail ? 'merchant' : 'custom')}} placeholder={merchantEmail || "charity@example.org"} required={!merchantEmail} className="w-full px-4 py-2 mt-6 text-base bg-white border rounded-md border-slate-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        {merchantEmail && emailSource === 'merchant' && (<p className="mt-2 text-xs text-green-600">Using your Square account email.</p>)}
                      </div>
                    </div>
                ...
                <div className="sticky top-20">
                  <div className="p-8 bg-white border rounded-xl border-slate-200">
                    ...
                    <div className="mt-6 space-y-4">
                      <div className="flex justify-between"><span className="text-slate-600">Base plan ({selectedPlan})</span><span className="font-medium text-slate-800">${basePrice}</span></div>
                      {deviceCount > 1 && (<div className="flex justify-between pb-4 border-b border-slate-200"><span className="text-slate-600">{deviceCount - 1} additional device{deviceCount > 2 ? 's' : ''}</span><span className="font-medium text-slate-800">${extraDevicePrice}</span></div>)}
                      <div className="flex items-center justify-between pt-4 font-semibold text-slate-900"><span className="text-lg">Total</span><span className="text-2xl">${totalPrice}<span className="text-base font-medium text-slate-500">/{selectedPlan === 'monthly' ? 'mo' : 'yr'}</span></span></div>
                    </div>
                    <div className="mt-8">
                      <label htmlFor="promo-code" className="block text-sm font-medium text-slate-700">Promo Code</label>
                      <input type="text" id="promo-code" value={promoCode} onChange={(e) => setPromoCode(e.target.value)} placeholder="Enter code" className="w-full px-3 py-2 mt-1 text-sm bg-white border rounded-md border-slate-300 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                    </div>
                  </div>
                  ...
                  <div className="mt-8">
                    <button type="submit" disabled={isLoading || !squareLoaded || !squareConfig} className="flex items-center justify-center w-full px-6 py-4 text-base font-semibold text-white bg-indigo-600 rounded-lg shadow-sm hover:bg-indigo-700 disabled:bg-slate-400 disabled:cursor-not-allowed">
                      {isLoading ? (<><Spinner size="5" color="white" /><span className="ml-3">Processing...</span></>) : (`Subscribe for $${totalPrice}/${selectedPlan === 'monthly' ? 'mo' : 'yr'}`)}
                    </button>
                  </div>
                  <div className="mt-6 text-center">
                    <a href={`shulpad://subscription/cancelled?merchant_id=${merchantId}`} className="text-sm font-medium text-slate-600 hover:text-indigo-600">← Return to ShulPad</a>
                  </div>
                </div>
              </aside>
            </div>
...
export default function CheckoutPage() {
  // The Suspense component is used to show a fallback UI while the page content is loading.
  return (
    <Suspense fallback={
      <div className="flex items-center justify-center min-h-screen bg-slate-50">
